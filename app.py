...
if perfil == "Coordenação" and acesso_liberado:
    ...
    if not df.empty and "Link_Arquivo" in df.columns:
        ...
        else: st.info("Nenhuma prova enviada.")
    ...
    if not df.empty:
        for _, r in df.iterrows():
            try:
                d, m, y = r['Data'].split('-')
                evs.append({"title": f"{r['Turma']}: {r['Disciplina']}", "start": f"{y}-{m}-{d}", "end": f"{y}-{m}-{d}", "color": "#3D5AFE" if r['Status'] == 'Concluído' else "#FF9100"})
            except: continue
    ...
    if not df.empty:
        ...
        df_f = st.selectbox("1. Sua Disciplina", ["Selecione..."] + discs)
        if df_f != "Selecione...":
            pends = df[(df['Disciplina'] == df_f) & (df['Status'] == 'Pendente')]
            if pends.empty: st.info("Sem pendências.")
            else:
                opts = {f"{row['Turma']} ({row['Data']})": row['ID'] for _, row in pends.iterrows()}
                sel = st.selectbox("2. Selecione a Turma", list(opts.keys()))
                id_sel = opts[sel]
                with st.form("f_prof", clear_on_submit=True):
                    ...
                    if st.form_submit_button("Salvar"):
                        if cont and arq:
                            link = upload_to_drive(arq, f"Prova_{df_f}_{id_sel}.pdf")
                            if link:
                                df.loc[df['ID'] == id_sel, ['Conteudo', 'Status', 'Link_Arquivo']] = [cont, 'Concluído', link]
                                conn.update(data=df)
                                st.success("Enviado!")
                                st.rerun()
